...
- **AI Agent development/research** → `ai-agents/research/`
- **Architecture/design patterns** → `architecture/patterns/`
- **Market research/industry** → `market-research/`
- **Multi-agent systems** → `multi-agent-systems/`
- **Personal learning/exploration** → `personal/learning/`
...
SESSION_ID=$(python data_sources/claude_code/get_session_id.py --quiet)
```
...
**Generated by:** Claude Code session `SESSION_ID` (YYYY-MM-DD)
```
...
ls -1 target-folder/ | grep "^[0-9][0-9]_" | tail -1
...
3. Analyze: "Video discusses AI agent introspection capabilities"
4. Recommend: ai-agents/research/ (AI agent development topic)
5. Check last file number: 01_security_pattern.md
6. Create: 02_introspection_research.md
7. Generate document with:
   - Thesis covering methodology, implications, applications
...
```
User: "Document this industry webinar on AI agents: https://www.youtube.com/watch?v=XYZ789"

Assistant workflow:
1. Extract transcript
2. Analyze: "Industry analysis of AI agent market trends"
3. Recommend: market-research/
4. Create: 03_ai_agent_market_2025.md
...
3. If personal → personal/learning/
4. If team → determine appropriate team knowledge subfolder
```
...
- **get_session_id.py:** Claude Code session ID extraction
  - Location: `data_sources/claude_code/get_session_id.py` (in your project)

### File Locations
- **Temporary transcripts:** `/tmp/` (auto-cleaned)
- **Target folders:** Organized by project structure (customizable)
- **Knowledge Framework guide:** See knowledge-framework skill documentation
...
❌ **Wrong session format:** Not including session UUID in attribution
...
**Meta Note:** This skill automates preservation of external video knowledge into your repository with proper academic rigor and organizational structure.